...
struct CollectiveEpilogue {
  ...
  CUTLASS_DEVICE void load_denoise_tail(
    ...
    // BN-11: threadIdx.x < 32 identifies warp 0 of warpgroup 0 directly,
    // saving one __shfl_sync call vs. the original warp_idx_in_warpgroup check.
    int lane_predicate = cute::elect_one_sync();
    if (threadIdx.x < cutlass::NumThreadsPerWarp && lane_predicate) {
